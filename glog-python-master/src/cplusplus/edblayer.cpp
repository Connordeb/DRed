...
static PyObject* edblayer_get_term_name(PyObject* self, PyObject *args);


static PyObject* edblayer_add_predicate(PyObject* self, PyObject *args);
...
static PyMethodDef EDBLayer_methods[] = {
    ...
    {"get_term_name", edblayer_get_term_name, METH_VARARGS, "Get textual name of a term." },
    {"add_predicate", edblayer_add_predicate, METH_VARARGS, "Add predicate name to EDB predicates" },
...
static PyObject* edblayer_add_predicate(PyObject* self, PyObject *args)
{
    const char *predName;
    glog_EDBLayer *s = (glog_EDBLayer*)self;
    if (!PyArg_ParseTuple(args, "s", &predName))
        return NULL;
    std::string sPredName(predName);
    uint64_t predId = s->e->addEDBPredicate(sPredName);
    std::vector<std::vector<std::string>> rows;
    s->e->addInmemoryTable(sPredName, predId, rows);
    
    return PyLong_FromLong(predId);
...
PyObject* edblayer_get_term_name(PyObject* self, PyObject *args)
{
    size_t termId;
    ...
    auto s = ((glog_EDBLayer*)self);
    auto termNameStr = s->e->getDictText(termId);
